...
The public DevNet Sandbox is a single always-on instance that everyone shares. The reserved instances are separate but reset every two to three days, and a reset silently changes the device's state: your branches vanish, the hostname returns to factory, and interface names can differ between instances (`GigabitEthernet0/0/0` on one, `GigabitEthernet0/1/0` on another). Every deploy now starts with a pre-flight gate: TCP probes on the RESTCONF port and (for host deployments) port 22, plus a RESTCONF hostname read. When the planes are down the deploy waits and retries (5s then 15s) before giving up, because a cycling sandbox usually comes back within a minute; when it gives up, the abort card says exactly which plane was unreachable instead of a generic SSH error. Every skipped or transport-failed attempt is logged as a WARN event under the PROVISION source, and the abort card states how many of the three attempts ran. The live hostname is recorded after each successful deploy, and if it changes between deploys the card warns "device identity changed — likely a sandbox reset". When a provisioning action or a scan fails for reasons that make no sense, check whether the device was reset first: the hostname probe and the `last_seen_hostname` setting reveal it. The remedy is to re-run the setup step against the current instance and adjust the interface names in `config/branches.yaml`.

The always-on instance's SSH plane has been observed flapping after a reset: TCP probes and even RESTCONF succeed while real SSH connections are refused for several minutes ("SSH connection failed: TCP connection to device failed"). Host deployments depend on SSH because the DHCP lease can only be pushed over the CLI, so such windows make add-PC fail even though the rest of the fabric looks healthy. The SSH fallback retries three times with short pauses to ride out brief flaps, and the whole deploy retries twice more after that; if all attempts fail, wait a minute or two (the plane recovers on its own) and re-run. Running two application instances at the same time doubles the session load on the shared sandbox and makes these windows likelier — keep a single instance open.
